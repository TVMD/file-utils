...
fn build_editor_content(
    ...
    content.push_str("# Dedup — Interactive Mode\n");
    content.push_str("#\n");
    ...
    for (i, group) in groups.iter().enumerate() {
        ...
        for &idx in group.iter() {
            ...
            let sim = if idx == keep_idx {
                100.0
            } else {
                jaro_winkler(&normalized[keep_idx], &normalized[idx]) * 100.0
            };

            // Default all to keep — user changes to 'd' to delete
            content.push_str(&format!("k {} # {:.0}% match\n", name, sim));
        }
        content.push('\n');
    }
...
fn open_editor(content: &str) -> io::Result<String> {
    ...
    let tmp_path = tmp_dir.join("fus-dedup.txt");
